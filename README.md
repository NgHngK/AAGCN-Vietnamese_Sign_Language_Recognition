...
# Setup
Download all the files and run the jupyter file Total (VSL). This file will do almost the entire training process, ranging from extracting the keypoints to performing interpolation, training the model, and evaluating it. All the other files are the supplement for the model part. 

Change the path of the input dataset. The code will read all videos in the folder. The dataset folder follows this structure:
<pre>
  dataset/
├── video1.mp4
├── video2.mp4
...
1. Train new.
2. Train continuously from the model that was pre-trained on the AUTSL dataset.
